...
本机完成 Claude CLI 登录后即可使用。`claude.model`、`claude.subagentModel`、`claude.apiKey`、`claude.baseUrl` 均为选填；填写后会把对应配置传给 Claude CLI 子进程，覆盖其默认值；留空则一切以 `~/.claude/settings.json` 为准。
