...
本机完成 Claude CLI 登录后即可使用。同时支持官方和第三方 Anthropic 兼容 API：使用官方服务无需额外配置，使用第三方 API 则填写 `claude.apiKey` 和 `claude.baseUrl`。`claude.model`、`claude.subagentModel`、`claude.apiKey`、`claude.baseUrl` 均为选填；填写后会把对应配置传给 Claude CLI 子进程，覆盖其默认值；留空则一切以 `~/.claude/settings.json` 为准。
